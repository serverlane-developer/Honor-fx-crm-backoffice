...
const LoginHistory = () => {
  ...
  const getLoginHistory = async ({ orderBy, dir, limit, skip, searchText } = {}) => {
    try {
      ...
      const endpoint = apiConstants.ADMIN_LOGIN_HISTORY;
  ...
  const columns = [
    ...
    {
      title: "2FA Enabled",
      dataIndex: "two_factor_authenticated",
      key: "two_factor_authenticated",
      render: (value) => (value ? "Yes" : "No"),
    },
    {
      title: "Login Success",
      dataIndex: "attempt_success",
      key: "attempt_success",
      render: (value) => <Tag color={value ? "success" : "error"}>{value ? "Yes" : "No"}</Tag>,
    },
    {
      title: "Message",
      dataIndex: "attempt_status_char",
      key: "attempt_status_char",
    },
  ];
